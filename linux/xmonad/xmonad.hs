...
myKeys conf@XConfig { XMonad.modMask = modm } =
    ...
    -- Programs
           ...
           , ( (modm, xK_space)
             , spawn "~/.config/rofi/launchers/colorful/launcher.sh"
             )
           , ( (0, xK_Print)
             , spawn
                 "spectacle -bmc -o ~/Pictures/$(date +%Y-%m-%d_%H-%M-%S).png"
             )
           , ( (controlMask, xK_Print)
             , spawn
                 "spectacle -brc -o ~/Pictures/$(date +%Y-%m-%d_%H-%M-%S).png"
             )
           , ((modm, xK_backslash), kill)
           , ((modm .|. controlMask, xK_backslash), killAll)
           , ((modm .|. shiftMask, xK_p), io exitSuccess)
           , ((modm, xK_p), spawn "~/.xmonad/recompile")
           ,
      -- Layouts
      ...
      -- Swap adjacent windows
             ((modm .|. controlMask, xK_l), withFocused (\w -> swapOrSend w R))
           , ((modm .|. controlMask, xK_h), withFocused (\w -> swapOrSend w L))
           , ((modm .|. controlMask, xK_k)    , windowSwap U True)
           , ((modm .|. controlMask, xK_j)    , windowSwap D True)
           ,
      ...
      -- Linked workspaces
           ...
           , ( (modm .|. controlMask, xK_i)
             , removeAllMatchings defaultMessageConf
             )
           ,
      -- Experimental
             ((modm, xK_g), restart "/home/hy/kmonad" True)
           ]
        ...
        ++ [ ((modm .|. m, k), a i)
           | (a, m) <-
               [ (switchWS (windows . W.greedyView) defaultMessageConf, 0)
               , ( switchWS (\x -> windows $ W.shift x . W.greedyView x)
                            defaultMessageConf
                 , shiftMask
                 )
               ]
...
isMapped win = withDisplay $ \dpy ->
    io $ (waIsUnmapped /=) . wa_map_state <$> getWindowAttributes dpy win

myMouseBindings XConfig { XMonad.modMask = modm } = M.fromList
    [ ( (modm, button3)
      , \w -> focus w >> Flex.mouseWindow Flex.resize w >> ifClick
    ...
    , ((0, 8), \w -> swapOrSend w R)
    , ((0, 9), \w -> swapOrSend w L)
    ]
...
instance Transformer DECORATIONS Window where
    transform _ x k = k
        (windowSwitcherDecorationWithImageButtons shrinkText myTheme x)
...
myTheme = defaultThemeWithImageButtons
    ...
    , fontName            =
        "-monotype-noto serif-medium-r-normal--0-0-0-0-p-0-microsoft-cp1252"
    }
...
myLayout =
    lessBorders Never
        ...
        $ minimize
        $ BW.boringWindows
        $ trackFloating (noBorders Full ||| tiled ||| Grid)
  where
...
myEvHook = mconcat [minimizeEventHook]

myEventHook = refocusLastEventHook <+> myEvHook
    where refocusLastEventHook = refocusLastWhen isFloat
